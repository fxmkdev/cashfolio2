...
import { isAreaHistoryMetric } from "./-chart-data";
import type { HistoryChartDatum } from "./-chart-data";
...
export {
  ...
  type HistoryOpeningBalancePoint,
  type HistoryVisibleRange,
} from "./-chart-data";

type BarHistoryMetric = Exclude<
  HistoryMetric,
...
export function createHistoryChartOptions(args: {
  ...
  const rangeButtons = getHistoryRangeButtons(args.periodMode);
  const rangeControlStyles = getHistoryRangeControlStyles(args);
  const selectedMetricLabel = getHistoryMetricLabel(args.selectedMetric);
  ...
  const getAreaTooltipHeading = (datum: HistoryChartDatum) =>
  ...
  const useRegularTimeAxis = isAreaHistoryMetric(selectedMetricKey);
    ...
    : [
        {
          ...
          itemStyler: ({ datum }: { datum: unknown }) => {
            ...
            const metricValue =
              (datum as HistoryChartDatum)[selectedMetricKey] ?? 0;
            const fill =
              metricValue > 0
                ? positiveFillColor
                : metricValue < 0
