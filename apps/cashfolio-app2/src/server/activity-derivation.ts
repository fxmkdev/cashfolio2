...
export type ActivityDerivedBooking = {
  id: string;
  date: Date;
...
export type ActivityDerivedRow = {
  id: string;
  transactionId: string;
  bookingValue: number;
  date: string;
...
export function deriveActivityRows(args: {
  bookings: ActivityDerivedBooking[];
}): {
  rows: ActivityDerivedRow[];
} {
  return {
