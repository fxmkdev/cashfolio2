...
export type HistoryMetricScopeFilter = {
  metric: HistoryScopedMetric;
  scope: HistoryScopeSelection;
};

export function buildBalanceHistoryScopeAmountMaps(args: {
  accounts: Array<{
    id: string;
    name: string;
    groupId: string | null;
...
export function resolveScopedMetricValue(args: {
  ...
  amountByMetric: Partial<
    Record<HistoryScopedMetric, Map<string, BreakdownHierarchyAccumulatorItem>>
  >;
  ...
  const parsedScope = parseHistoryScopeSelection(args.metricScopeFilter.scope);
  if (!parsedScope) {
    return undefined;
  }
...
export function buildHistoryGainLossScopeOptions(args: {
  hierarchy: PeriodGainsLossesBreakdownNode[];
}): HistoryScopeOption[] {
  ...
  const appendNodes = (
    ...
    parentValue: HistoryScopeSelection | undefined,
    parentLabel: string | undefined,
  ) => {
    for (const node of nodes) {
      const value = node.id as HistoryScopeSelection;
...
export function buildHistoryScopeOptions(args: {
  amountByAccountId: Map<string, BreakdownHierarchyAccumulatorItem>;
  allAccountGroups: Array<{
    id: string;
    name: string;
