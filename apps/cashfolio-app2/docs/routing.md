...
- `$accountBookId/timeline/route.tsx` - timeline page showing monthly/yearly
  metric history as a full-page bar + cumulative line chart (legend toggles
  each series on/off)
  - Includes a metric switcher (`totalReturn`, `savings`, `income`,
    `expenses`, `gainsLosses`).
  - Cumulative line rebases to the currently visible range so
    navigator/range-button/zoom interactions update the running baseline for
    the selected metric.
